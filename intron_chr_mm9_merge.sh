...
for (( row=0;row<index;row++)); do
    ...
    bsub -M 16000000 -R "rusage[mem=16000]" "python exonintron_counter.py -a ${prefixarray[row]} $delta intronsbed.txt ${prefixarray[row]}/${prefixarray[row]}_intronreads.txt"
done
